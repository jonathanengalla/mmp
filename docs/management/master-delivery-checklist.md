...
| Code | Name | Status | Notes |
| --- | --- | --- | --- |
| FIN-01 | Finance Dashboard Contract & Metrics Alignment | 🟢 | **Complete end-to-end:** Finance summary endpoint (`/api/billing/admin/finance/summary`) with time window support, source breakdown (DUES/DONATION/EVENT/OTHER), status mapping (OUTSTANDING/PAID/CANCELLED), zero-amount exclusion, tenant scoping. Response includes self-describing range labels. Backend tests (`npm run test:fin-01`). Frontend aligned via UIR-03 with regression tests (`npm test finance-helpers`, `npm test admin-finance-dashboard`). **Post-implementation fixes (2025-12-13):** Fixed Revenue Breakdown invoice count mismatch - enhanced source normalization (handles all variations: EVT/EVENT, DUES/DUE, DONATION/DON, OTHER/OTH, null/empty), fixed double-counting of partially paid invoices, added validation and debug logging. Revenue Breakdown now correctly shows all invoices matching Status Breakdown totals. Spec: `docs/specs/finance/FIN-01-finance-dashboard-contract.md`. **Note:** Refactored to use PAY-10 Allocation model - queries use allocations instead of direct payments. |
